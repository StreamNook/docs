...
description: Every field of plugin.toml, the permission contract, the capability tiers, and install validation.
---
...
| Field | Type | Constraints |
|---|---|---|
| `kind` | string | `"process"` (a separate executable) or `"ui"` (an in-app interface module, see [Plugin UI](/docs/plugin-ui)). `"wasm"` is reserved and rejected |
...
| `ui_entry` | string | Optional, `process` only: path of an in-app UI module the sidecar ships alongside itself (a hybrid plugin). Same path rules as `entry`. See [Plugin UI](/docs/plugin-ui) |

<Note>
For `kind = "ui"` the `[capabilities]` and `[contributes]` blocks stay empty. The module contract in [Plugin UI](/docs/plugin-ui) is the complete surface. A hybrid `process` plugin that sets `ui_entry` keeps its capabilities and contributions for the sidecar. The `ui` kind and `ui_entry` were added after protocol v1 froze. Both are additive (the plugin's `host_min` must be a version that knows them, and older hosts fail closed on the unknown value or field).
</Note>

## `[capabilities]`

The whole permission contract. The consent dialog renders exactly this block in plain language (see [Plugin Capabilities](/docs/plugin-capabilities) for the vocabulary and the rendered strings).

| Field | Type | Meaning |
|---|---|---|
| `events` | string array | Events the plugin may subscribe to. Subset of the event list in [Protocol](/docs/api-protocol) |
...
Optional. The named hooks the plugin fills so the native UI can delegate to it. See [Hooks](/docs/api-hooks) for the full mechanism.
...
The manifest itself carries no signature block. Signatures are detached and live alongside the artifact. The author's public key is pinned through the index and the first-install flow. This avoids a self-referential artifact (the manifest travels inside the signed artifact) and keeps one verification path. See [Plugin Signing](/docs/plugin-signing).
...
<Info>
The tier is curation metadata: it determines where a plugin may be listed (see [Plugin Signing](/docs/plugin-signing)). Its user-facing presentation is a neutral badge. The consent flow ([Plugin Capabilities](/docs/plugin-capabilities)) is the same calm, capability-focused dialog for every tier. The official index lists the curated A and B tiers; heavier or specialized add-ons live in community sources.
</Info>

## Validation order at install

<Steps>
<Step title="Artifact hash">
Artifact hash matches the index entry.
</Step>
<Step title="Signature">
Detached signature verifies against the author key (pinned via the index and trust-on-first-use).
</Step>
<Step title="Manifest parse">
Manifest parses, all constraints above hold.
</Step>
<Step title="Index match">
Manifest `id`, `version`, and `tier` exactly match the index entry. Any mismatch blocks install.
</Step>
<Step title="Host support">
`host_min` is satisfied and `runtime.kind` plus `runtime.transport` are supported.
</Step>
<Step title="Consent">
The consent dialog is shown. The user grants capabilities (possibly a subset).
</Step>
<Step title="Register">
The plugin is unpacked into its own directory and registered.
</Step>
</Steps>
...
The permission vocabulary and the strings the consent dialog renders.
</Card>
<Card title="Hooks" icon="plug" href="/docs/api-hooks">
How `[contributes]` entries wire the native UI to your plugin.
</Card>
<Card title="Plugin UI" icon="code" href="/docs/plugin-ui">
The module contract for `ui` and `ui_entry` interface plugins.
</Card>
<Card title="Signing" icon="book" href="/docs/plugin-signing">
Detached signatures, key pinning, and tier-based distribution.
</Card>
<Card title="Protocol" icon="terminal" href="/docs/api-protocol">
The event list and transport that back the capability fields.
</Card>
</CardGroup>
